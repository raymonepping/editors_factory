...
9. **`vault-agent`'s own token renewal can silently stall without falling back to re-authentication — a documented class of upstream Vault Agent behavior (several related `hashicorp/vault` GitHub issues), not something this project's own config can fully prevent.** Found live, twice, in sessions running many hours: `vault-agent` kept reporting "healthy" and its logs showed regular `renewed auth token` entries, then stopped entirely — no further renewal, no re-auth attempt, no error logged. The token it was still handing out via `/vault/secrets/token` had genuinely died (confirmed with a direct `auth/token/lookup-self` call against it: 403 "invalid token"). **Hardened (`prompts/hardening/01_00`):** both blind spots that let this go unnoticed for hours are now real checks, not file-existence checks — `vaultHealthCheck()` (`backend/src/vault.js`) and `vault-agent`'s own Docker healthcheck (`compose/vault/compose.yaml`) both now perform a genuine `auth/token/lookup-self` call, confirmed live to flip `vault.ok`/container health to false immediately on a revoked token (previously: silently `true` for hours, since a dead token is still a non-empty file). The recreate-cascade this gotcha used to warn about (`--force-recreate vault-agent` also recreating `vault-s`/`vault-1`, sealing `vault-s`) is also fixed at the root: `vault-agent`'s `depends_on: vault-1: condition: service_healthy` was removed (Vault Agent's own auto_auth already retries connecting with backoff when `vault-1` isn't ready — confirmed live, including across a full cold `make down && make up`) — podman-compose was treating that `depends_on` as a real engine-level requirement, which is what pulled `vault-1` (and transitively `vault-s`) into every `--force-recreate vault-agent`. `factory-api`'s own AppRole `token_max_ttl` was also raised from 4h to 24h (`terraform/vault-platform/auth.tf`) to reduce how often a full renewal-to-reauth cycle has to complete without hitting whatever triggers this — risk reduction, not a fix. Recovery is now one command: `make vault-agent-recover` (`scripts/vault-agent-recover.sh`) — force-recreates `vault-agent`, waits for its real healthcheck, checks/unseals the cluster defensively (should no longer be needed, but the script stays cautious in case this ever regresses), recreates `factory-api`, then the agents (SSE, gotcha #3). Idempotent — safe to run even when nothing is actually broken.
