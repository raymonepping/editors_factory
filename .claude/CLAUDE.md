...
8. **`vault-agent` (`factory-vault_agent`, `compose/vault/vault-agent/`) is not part of `make vault-up`/`vault-bootstrap.sh` at all** — found live after a `make down && make up` cycle (following a Podman machine restart) silently left it torn down while the rest of the stack came back fine. Nothing restarts it automatically; `factory-api` keeps running for a while on its last-rendered token, then starts failing Vault KV reads at startup with "permission denied / invalid token" once that token goes stale with nothing renewing it. Fix: `./scripts/compose.sh vault up -d` (brings up the whole `compose/vault/compose.yaml` stack, vault-agent included — safe to run even when vault-1/2/3/s are already healthy), then recreate `factory-api` to pick up the fresh token.
