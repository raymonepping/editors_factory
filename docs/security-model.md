...
### Ledger/mutation atomicity — assessed, not pursued

The idempotency-ledger claim and the domain mutation it guards are deliberately not one PostgreSQL transaction, and that is worth stating plainly rather than leaving it as an unexplained implementation detail. `claimBusinessEffect()`'s own `INSERT ... ON CONFLICT DO NOTHING` runs on the backend's `factory-backend-role` connection; the mutation it guards runs on Agent C's own Vault-issued `factory-bad-role`/`factory-good-role` connection. Those are two different PostgreSQL roles by design — the agent's role is deliberately never granted write access to evidence tables (see "Dynamic database identities" above) — so spanning both in one transaction would mean widening that grant, which weakens the authority boundary this project exists to demonstrate for the sake of bookkeeping convenience.

The claim step itself is still atomic (the `INSERT ... ON CONFLICT` decides which attempt "wins" the effect before either one touches domain data, which is what actually matters for correctness under a normal retry). The real, narrow residual window is a genuine process crash between the claim committing and the mutation running, not anything a fault-injection mode simulates — none of `fail_before_mutation`, `fail_after_mutation`, or `lock_timeout` land there. A crash in that exact window would leave the ledger row claimed with a null result, and a later retry would report "already applied by a prior attempt" for a mutation that, in that one scenario, never actually ran.

A transactional outbox pattern would close that window, at the cost of either the grant-widening above or a message broker this project has already rejected once on the record (`docs/project-history.md`'s "Local and auditable by design": custom evidence tables were chosen specifically to avoid that dependency). For a local demonstration whose thesis is about authority boundaries, not distributed-transaction correctness, that trade isn't worth making. **Recommendation: not now.** If this project ever needs to survive a mid-mutation process crash without operator intervention as a hard requirement (not merely a nice-to-have), an outbox pattern confined to the backend's own connection, writing the mutation's own intent durably before attempting it, is the narrower fix to revisit first — before reaching for a shared transaction across the authority boundary.
