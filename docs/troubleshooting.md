...
During implementation, backend database authentication failed twice after a long run and recovered after the API restarted. The root cause is now established (`prompts/hardening/02_00_backend_db_credential_reliability.md`): `GET /api/health`'s own db check ran `pool.query("SELECT 1")`, and a `pg.Pool` reuses already-open connections — PostgreSQL does not retroactively kill a session just because the role behind it was later dropped by Vault's own lease revocation, so a query landing on one of those still-open connections kept succeeding even after the credential backing the pool had been fully revoked, masking the failure indefinitely. `/api/health`'s db check now opens a throwaway, never-pooled connection with the current credential on every call (`backend/src/db.js`'s `verifyCredentialFresh`), so `db.ok` means the credential can open a real connection right now, not "the pool happened to have a working one already open." The out-of-band renewal this triggers on failure (`db.js`'s `renewNow`) is unchanged. The proactive side also moved off a single long-delay `setTimeout` (due once, at 80% of the credential's TTL) onto a short-interval heartbeat (`RENEWAL_HEARTBEAT_MS`, 60s) that re-evaluates the credential's age every tick — self-correcting by construction, since a single missed tick isn't a lost callback the way the old design's single timer firing exactly once, or not at all with no trace, was. Confirmed live: revoking the pool's own credential (`ALTER ROLE ... NOLOGIN` on the underlying Postgres role — the same observable effect Vault's own revocation produces) was caught and self-healed automatically within one container healthcheck interval, no restart needed, with both the detection and the recovery logged (`podman logs factory-api | grep '\[db\]'`). Treat a `db.ok: false` that persists across several health checks as worth investigating: capture the logged renewal attempts before restarting the API.
